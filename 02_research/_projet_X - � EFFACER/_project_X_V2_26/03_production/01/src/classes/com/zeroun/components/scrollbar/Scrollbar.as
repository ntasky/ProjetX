package com.zeroun.components.scrollbar
{
	...
	public class Scrollbar extends MovieClip
	{
		...
		public function initialize(	__target:Object,
									__mask:Sprite = null,
		...
		public function setProperties(__x:Number, __y:Number, __h:Number = Number.MAX_VALUE):void
		{
		...
		public function set enable(__enable:Boolean):void
		{
			...
			_arrow1.enabled = __enable;
			_arrow1.gotoAndStop(__frame);
			_arrow2.enabled = __enable;
			_arrow2.gotoAndStop(__frame);
			_background.enabled = __enable;
			_background.gotoAndStop(__frame);
			_cursor.enabled = __enable;
			_cursor.gotoAndStop(__frame);
			
			_setListenners(__enable);
		}
		
		public function update(__x:Number = -1, __y:Number = -1):void
		{
			var wasEnabled:Boolean = enable;
			__x = (__x > -1 ) ? __x : x;
			__y = (__y > -1 ) ? __y : y;
			setProperties(__x, __y);
			_checkScrollValue();
			if (enable) _updateTargetPosition();
			else if (!enable && wasEnabled) _resetTargetPosition();
		}
		...
		protected function _initialize(__event:TimerEvent = null):void
		{
			...
			{
				case TEXT_TYPE:
					setProperties((_target.x + _target.width), _target.y);
					_checkScrollValue();
					break;
				case OBJECT_TYPE:
					setProperties((_mask.x + _mask.width), _mask.y);
					_checkScrollValue();
					break;
				case ARRAY_TYPE:
					setProperties((_mask.x + _mask.width) - _scroll.width, 0);
					_checkScrollValue();
					break;
				default:
				...
				switch (_scrollType)
				{
					...
					case OBJECT_TYPE:
						if (_target.height > _mask.height)
						{
							...
							if (!_fixedCursorSize)
							{
								_cursor.height = Math.max(_cursorHeight, Math.round(_backgroundHeight / (_target.height / _backgroundHeight)));
							}
						}
						else
						{
						...
						_minTargetPosition = Math.round(_mask.y - _target.height + _mask.height)
		...
		// NOTE: this is used in case we don'T want to scroll on the full size of the target
