package core
{
	...
	public class Main extends MovieClip implements ILoadable
	{
		...
		private function _reveal() : void
		{
			...
			_menu = new Menu(XML(LoaderAccessor.getXML("content.xml").pages), RollDownMenuItem, Content.locale, 100, 0);
			_menu.x = 10;
			_menu.y = 10;
			...
			if (Config.SWF_ADDRESS_ENABLED) SWFAddress.addEventListener(SWFAddressEvent.CHANGE, _onChangeSWFAddress) else _goToPage("");
			
			// reveal
			TweenLite.from(this, 1, { autoAlpha:0 } );
		...
		private function _goToPage(__address:String):void
		{
			...
			if (Config.SWF_ADDRESS_ENABLED)
			{
				...
				else pageId = SWFAddress.getValue();
				
				// add logic to navigate from one page to another here...
				traceDebug("Page content for " + pageId);
		...
		private function _onInitSWFAddress(__event:SWFAddressEvent):void
		{
			...
			traceDebug ("_onInitSWFAddress!! >> " + SWFAddress.getValue() + ":" + _waitForSWFAddress);
		}
		...
		private function _onChangeSWFAddress(__event:SWFAddressEvent):void
		{
			...
			if (Config.MULTI_LANGUAGE)
			{
				...
				if (tmpLocale == Content.locale) _goToPage(__event.value);
				else navigateToURL(new URLRequest(Config.SITE_URL + SWFAddress.getValue()), "_self");
			}
			else _goToPage(__event.value);
		}
		...
		private function _onClickMenu(__event:MenuEvent):void
		{
			...
			if (Config.SWF_ADDRESS_ENABLED && Config.MULTI_LANGUAGE) addressLanguage = "/" + Content.locale;
			
			// when using SWFAddress, _goToPage is called automatically through _onChangeSWFAddress
			if (Config.SWF_ADDRESS_ENABLED) SWFAddress.setValue(addressLanguage + __event.itemId) else _goToPage(addressLanguage + __event.itemId);
		}
		...
		private function _onDefaultSWFAddress():void
		{
			// add language to the address
			var addressLanguage:String = "";
			if (Config.MULTI_LANGUAGE) addressLanguage = "/" + Config.DEFAULT_LOCALE;
