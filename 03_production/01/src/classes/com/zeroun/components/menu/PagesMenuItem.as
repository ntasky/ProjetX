package com.zeroun.components.menu
{
	...
	public class PagesMenuItem extends MovieClip implements IMenuItem
	{
		...
		public function setProperties(__id:String, __label:String, ... args):void
		{
			...
			if (args[0] != null && args[0].length() > 0)
			{
				...
				for (var i:int = 0; i < args[0].length(); i++ )
				{
					...
					item.setProperties(args[0][i].@id, (parent as PagesMenu).getLabel(args[0][i].label));
					...
					(parent as PagesMenu).addSubItemsById(args[0][i].@id, item);
					(parent as PagesMenu).addItemsById(args[0][i].@id, this);
				}
		...
		{
			return mcMenuItem.mcBackground.height;
		}
		
		public function get visibleWidth():int
		{
			return mcMenuItem.mcBackground.width;
		}
