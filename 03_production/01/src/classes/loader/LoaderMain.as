package loader
{
	...
	import loader.*;
	import loader.loadinganimation.*;

	
	public class LoaderMain extends MovieClip implements ILoaderMain
	{
		...
		{
			// setup the stage
			stage.scaleMode = LoaderConfig.STAGE_SCALE_MODE;
			stage.align = LoaderConfig.STAGE_SCALE_ALIGN;
		...
		{
			_loadingAnimation.update(LoaderConfig.CORE_SWF_PERCENTAGE * event.bytesLoaded / event.bytesTotal);
		}
		...
		private function _onCoreComplete(event:Event):void
		{
			...
			// wait for the content to be initializable (SWFAddress takes few frames to be initialized)
			addEventListener(Event.ENTER_FRAME, _checkContentStatus);
		}
		
		private function _checkContentStatus(__event:Event):void
		{
			if (_core.contentIsInitializable())
			{
				removeEventListener(Event.ENTER_FRAME, _checkContentStatus);
		...
		private function _onXMLProgress(event:ProgressEvent):void
		{
			...
			_loadingAnimation.update(LoaderConfig.CORE_SWF_PERCENTAGE + LoaderConfig.CORE_XML_PERCENTAGE * n);
		}
		...
		{
			var n:Number = (_nbFilesLoading - _filesToLoad.length + event.bytesLoaded / event.bytesTotal) / _nbFilesLoading;
			_loadingAnimation.update(LoaderConfig.CORE_SWF_PERCENTAGE + LoaderConfig.CORE_XML_PERCENTAGE + (100 - LoaderConfig.CORE_SWF_PERCENTAGE - LoaderConfig.CORE_XML_PERCENTAGE) * n);
		}
